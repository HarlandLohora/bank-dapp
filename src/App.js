...
function App() {
  ...
  const contractAddress = '0x3A647d7251e4A281D3a4096405B0097F0038513A';
  ...
  const getBankName = async () => {
    try {
      if (window.ethereum) {
        ...
        if (bankName) bankName = utils.parseBytes32String(bankName)
  ...
  const deposityMoneyHandler = async (event) => {
    try {
      if (window.ethereum) {
        ...
        await txn.wait()
        console.log("Depositing complete...", txn.hash);
  ...
  const withDrawMoneyHandler = async (event) => {
    try {
      ...
      if (window.ethereum) {
        ...
        const txn = await bankContract.withDrawMoneyHandler(myAddress, ethers.utils.parseBytes32String(inputValue.withdraw))
